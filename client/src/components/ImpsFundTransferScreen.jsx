    ...
    <div
      className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-[13px] font-bold ${
          ...
          : active
            ...
            : 'border-2 border-white/60 bg-transparent text-white/70'
      }`}
    >
      {done ? '✓' : n}
    </div>
...
function ProgressBar({ phase }) {
  const step = ['type_select', 'within_bank', 'other_bank'].includes(phase)
    ? 0
    : phase === 'review'
      ? 1
      : 2;
  ...
  return (
    <div className="mx-3 mt-3 overflow-hidden rounded-lg border border-slate-200 bg-[#eef6fc]">
      <div className="flex items-center justify-between border-b border-slate-200 px-3 py-1.5">
        ...
        <button type="button" className="text-[11px] font-semibold text-[#0a3d62] press">
          Change
        </button>
      </div>
...
function BankModal({ open, onClose, onSelect }) {
  ...
  return (
    <AnimatePresence>
      {open && (
        <motion.div
          className="absolute inset-0 z-[82] flex items-end bg-black/45"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          onClick={onClose}
          ...
          >
            <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3">
              ...
              <button
                type="button"
                onClick={onClose}
                className="text-xl leading-none text-slate-500 press"
              >
                ×
              </button>
            </div>
              ...
              {BANKS.map((b) => (
                <li key={b} className="border-b border-slate-100 last:border-0">
                  <button
                    type="button"
                    onClick={() => onSelect(b)}
                    className="w-full px-4 py-3 text-left text-[13px] text-slate-900 press hover:bg-slate-50"
                  >
                    {b}
                  </button>
                </li>
    ...
    >
      ...
      <div className="absolute top-10 left-0 right-0 flex flex-col items-center">
        <img
          ...
          alt="Indian Bank"
          className="h-10 max-w-[160px] object-contain opacity-90"
        />
      ...
      >
        <p className="text-[13px] font-semibold text-white/60 uppercase tracking-widest">
          Transferring
        </p>
      ...
      <div className="flex flex-col items-center gap-1 px-8">
        {PROCESSING_STEPS.slice(0, stepIdx + 1).map((s, idx) => (
          <motion.p
            ...
            animate={{ opacity: idx === stepIdx ? 1 : 0.38, y: 0 }}
...
export default function ImpsFundTransferScreen({
  onClose,
  lang,
  accounts,
  ...
  const setFvFromAgent = useCallback((next) => {
    ...
    const justSet = (id) =>
      next[id] !== undefined && next[id] !== (prev[id] ?? '') && String(next[id]).trim() !== '';
    ...
    if (justSet('payeeType')) {
      ...
      setPayeeType(pt);
      if (phaseRef.current === 'type_select') {
  ...
  /* validation */
  const withinValid =
    payeeName.trim().length >= 2 && /^\d{9,18}$/.test(payeeAccountNo) && Number(amount) > 0;
  const otherAccountValid =
    /^[A-Za-z]{4}0[A-Za-z0-9]{6}$/.test(ifsc) &&
    /^\d{9,18}$/.test(payeeAccountNo) &&
    Number(amount) > 0;
  const otherMobileValid =
    payeeBank.trim().length >= 2 && /^\d{10}$/.test(mobileNo) && Number(amount) > 0;
  const step1Valid =
    transferType === 'within'
      ? withinValid
      : payeeType === 'account'
  ...
  const handleBack = () => {
    if (phase === 'within_bank' || phase === 'other_bank') {
      setAiOpen(false);
      setPhase('type_select');
    } else if (phase === 'review')
      setPhase(transferType === 'within' ? 'within_bank' : 'other_bank');
  ...
  const handleDone = () => {
    setPhase('type_select');
    setTransferType('within');
    setPayeeType('account');
    setPayeeName('');
    setPayeeAccountNo('');
    setIfsc('');
    setPayeeBank('');
  ...
  return (
    ...
    >
      ...
      {/* ── Scrollable body ── */}
      <div
        className={`min-h-0 flex-1 overflow-y-auto no-scrollbar bg-[#f5f7fa] ${isFormPhase && aiPanelOpen ? 'pb-[clamp(160px,28vh,240px)]' : 'pb-24'}`}
      >
        {/* PHASE: type_select */}
        {phase === 'type_select' && (
          <>
            ...
            <div className="mx-3 mt-4">
              ...
              <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
                ...
                <button
                  type="button"
                  className="shrink-0 rounded border border-slate-300 bg-white px-4 py-1.5 text-[12px] font-semibold text-slate-700 press"
                >
                  Favourite Transac...
                </button>
              </div>
            ...
            <div className="mx-3 mt-4 overflow-hidden rounded-lg border border-slate-200 bg-white">
              ...
              <button
                type="button"
                onClick={() => setPhase(transferType === 'within' ? 'within_bank' : 'other_bank')}
                className="flex w-full items-center gap-2 px-4 py-3 text-[13px] font-semibold text-[#0a3d62] press hover:bg-slate-50"
              >
                <span className="flex h-5 w-5 items-center justify-center rounded-full border-2 border-[#0a3d62] text-[14px] font-bold leading-none">
                  +
                </span>
                SEND TO A NEW PAYEE
              </button>
            ...
            <div className="mx-3 mt-4 space-y-3">
              ...
              <div>
                ...
                <TInput
                  ...
                  onChange={(e) =>
                    setPayeeAccountNo(e.target.value.replace(/\D/g, '').slice(0, 18))
                  }
                  onBlur={() => {
                    if (!/^\d{9,18}$/.test(payeeAccountNo)) markInvalidField('payeeAccountNo');
                  }}
                  placeholder="Please Type Here...."
                  inputMode="numeric"
                  highlight={highlightField === 'payeeAccountNo'}
                />
              </div>
              <div>
                ...
                <TInput
                  ...
                  onBlur={() => {
                    if (payeeName.trim().length < 2) markInvalidField('payeeName');
                  }}
                  placeholder="Please Type Here...."
                  highlight={highlightField === 'payeeName'}
                />
              </div>
              <div>
                <FieldLabel required>Amount (₹)</FieldLabel>
                <div
                  className={`flex items-center gap-1 rounded border px-3 py-2 transition-colors ${amtHl ? 'border-amber-400 bg-amber-50 ring-1 ring-amber-400/50' : 'border-slate-300 bg-white focus-within:border-[#0a3d62] focus-within:ring-1 focus-within:ring-[#0a3d62]/30'}`}
                >
                  ...
                  <input
                    ...
                    onBlur={() => {
                      if (!amount || Number(amount) <= 0) markInvalidField('amount');
                    }}
                    placeholder="0.00"
                  />
                </div>
              ...
              <div>
                ...
                <TInput
                  value={remarks}
                  onChange={(e) => setRemarks(e.target.value.slice(0, 50))}
                  placeholder="Max 50 characters"
                  highlight={highlightField === 'remarks'}
                />
              </div>
            </div>
          </>
          ...
          <>
            ...
            <div className="mx-3 mt-4 space-y-3">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <Chip active={payeeType === 'account'} onClick={() => setPayeeType('account')}>
                    Account Number
                  </Chip>
                  <Chip active={payeeType === 'mobile'} onClick={() => setPayeeType('mobile')}>
                    Mobile Number
                  </Chip>
                </div>
              </div>
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      <TInput
                        ...
                        onBlur={() => {
                          if (!/^[A-Za-z]{4}0[A-Za-z0-9]{6}$/.test(ifsc)) markInvalidField('ifsc');
                        }}
                        placeholder="Please Type Here...."
                        highlight={highlightField === 'ifsc'}
                      />
                      <button
                        type="button"
                        className="shrink-0 rounded bg-[#0a3d62] px-3 py-2 text-[11px] font-bold text-white press"
                      >
                        Search IFSC
                      </button>
                    </div>
              ...
              {payeeType === 'mobile' && (
                <>
                  ...
                  <div>
                    ...
                    <div
                      className={`flex items-center overflow-hidden rounded border transition-colors ${highlightField === 'mobileNo' ? 'border-amber-400 bg-amber-50' : 'border-slate-300 bg-white focus-within:border-[#0a3d62] focus-within:ring-1 focus-within:ring-[#0a3d62]/30'}`}
                    >
                      ...
                      <input
                        className="min-w-0 flex-1 bg-transparent px-3 py-2 text-[13px] outline-none"
                        inputMode="numeric"
                        maxLength={10}
                        value={mobileNo}
                        onChange={(e) =>
                        ...
                        onBlur={() => {
                          if (!/^\d{10}$/.test(mobileNo)) markInvalidField('mobileNo');
                        }}
                        placeholder="Please Type Here...."
                      />
                    </div>
        ...
        {phase === 'review' && (
          <div className="mx-3 mt-3">
            ...
            <div className="mb-4 flex flex-col items-center rounded-2xl bg-[#0a3d62] py-5 text-center text-white shadow-lg">
              ...
              <p className="mt-1 text-[38px] font-bold leading-none">
                ₹{Number(amount || 0).toLocaleString('en-IN')}
              </p>
              <p className="mt-1 text-[12px] text-white/70">
                {transferType === 'within'
                  ? `To: ${payeeName} · ${payeeAccountNo}`
                  : payeeType === 'account'
                    ? `To: ${payeeAccountNo} · IFSC ${ifsc}`
                    : `To: +91 ${mobileNo} · ${payeeBank}`}
              </p>
            </div>
            {/* Details table */}
            <div className="overflow-hidden rounded-xl border border-dashed border-slate-600 bg-white">
              {[
                [
                  'Transfer Type',
                  transferType === 'within' ? 'Within Bank (NEFT)' : 'Other Bank (IMPS)',
                ],
                ...(transferType === 'within'
                  ? [
                      ['Payee Name', payeeName],
                      ['Account No.', payeeAccountNo],
                    ]
                  : payeeType === 'account'
                    ? [
                        ['IFSC', ifsc],
                ...
                <div
                  key={k}
                  className="grid border-b border-dashed border-slate-400 last:border-0"
                  style={{ gridTemplateColumns: 'minmax(0,40%) 1fr' }}
                >
                  <div className="bg-sky-100 px-3 py-2 text-[11px] font-bold text-slate-900">
                    {k}
                  </div>
            ...
            <p className="mt-2 text-center text-[10px] text-slate-400">
              Demo · No real funds transferred · Secured by NPCI
            </p>
          </div>
        )}
        ...
        {phase === 'success' && (
          <div className="relative flex flex-col items-center px-4 pt-6 text-center">
            ...
            <p className="relative z-10 text-[20px] font-bold text-slate-900">
              Transfer Successful!
            </p>
      ...
      <AnimatePresence>
        {showProcessing && (
          <ProcessingOverlay amount={amount} rrn={rrn} onDone={handleProcessingDone} />
        )}
      </AnimatePresence>
