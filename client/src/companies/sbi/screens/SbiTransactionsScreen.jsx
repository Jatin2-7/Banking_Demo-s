...
import { filterTransactionsByRange, formatPeriodLabel } from '../../../lib/transactionDateFilter.js';
...
export default function SbiTransactionsScreen({ onBack, initialDateFrom = null, initialDateTo = null }) {
  ...
  return (
    <div className="flex min-h-full flex-col bg-white">
      <header className="flex items-center gap-2 border-b px-3 py-3" style={{ borderColor: SBI.border }}>
        ...
        <button type="button" className="press p-1" style={{ color: SBI.purple }} aria-label="Notifications">
          <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth="1.8">
            <path d="M18 8a6 6 0 10-12 0c0 7-3 7-3 7h18s-3 0-3-7" />
          </svg>
        </button>
        <button type="button" className="press p-1" style={{ color: SBI.purple }} aria-label="Support">
          <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth="1.8">
      ...
      <div className="flex-1 overflow-y-auto px-3 pb-6">
        <p className="pt-3 text-[10px] font-semibold uppercase tracking-wide text-slate-500">Select Account</p>
        ...
        <div className="mt-3 flex items-center gap-2">
          ...
          <button type="button" className="press shrink-0 p-2" style={{ color: SBI.purple }} aria-label="Filter">
            <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth="1.8">
              <path d="M4 6h16M4 12h10M4 18h6" strokeLinecap="round" />
            </svg>
          </button>
          <button type="button" className="press shrink-0 p-2" style={{ color: SBI.purple }} aria-label="Sort">
            <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth="1.8">
        ...
        {groups.length === 0 ? (
          <div className="mt-8 rounded-xl border px-4 py-8 text-center" style={{ borderColor: SBI.border }}>
            <p className="text-[12px] font-semibold text-slate-700">No transactions found</p>
            <p className="mt-1 text-[10px] text-slate-500">
              {hasFilter ? `Nothing in ${periodLabel}. Try a different date range.` : 'Your statement is empty.'}
            </p>
          </div>
        ) : (
          groups.map((group) => (
                ...
                {group.items.map((txn, i) => {
                  ...
                  return (
                    <div key={`${txn.date}-${i}`} className="flex items-start gap-2.5 py-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-[11px] font-medium text-slate-800">{txn.desc}</p>
