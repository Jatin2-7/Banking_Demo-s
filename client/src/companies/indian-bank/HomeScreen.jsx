...
function TriColorUpiMark({ size = 22 }) {
  return (
    <div className="mx-auto flex shrink-0 overflow-hidden rounded shadow-sm" style={{ width: size, height: size * 0.7 }} aria-hidden>
...
function GoldQuickTile({ icon, label, onClick }) {
  return (
    ...
    >
      ...
      <span className="line-clamp-2 w-full flex-1 px-0.5 text-[9px] font-semibold leading-[1.15] text-white/95">{label}</span>
    </button>
  );
}
...
function ServiceTile({ icon, label, onClick }) {
  return (
    ...
    >
      <div className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-xl shadow-sm">{icon}</div>
...
function BottomNavItem({ icon, label, onClick }) {
  return (
    <button type="button" onClick={onClick} className="press flex w-full max-w-[4.5rem] flex-col items-center justify-end gap-1 justify-self-center">
      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-bank-gold text-base text-bank-nav shadow-md">{icon}</div>
  ...
  return (
    <div className="relative flex min-h-full flex-col pb-2" {...homeRageProps}>
      <header className="shrink-0 px-3 pb-2 pt-1.5">
        <div className="grid items-center gap-2" style={{ gridTemplateColumns: `${HEADER_SIDE} minmax(0, 1fr) auto` }}>
          <button
            type="button"
          ...
          <div className="flex shrink-0 flex-wrap justify-end gap-0.5 justify-self-end" style={{ maxWidth: '9.5rem' }}>
            ...
            <HeaderIconButton label="Log out">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <path d="M10 17H6a2 2 0 01-2-2V9a2 2 0 012-2h4M14 21l6-6-6-6M20 15H9" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </HeaderIconButton>
          </div>
        </div>
        ...
        <div className="mt-2.5 grid grid-cols-3 items-center gap-2 px-0.5">
          ...
          <div className="flex min-w-0 items-center justify-center gap-2">
            ...
            <button type="button" className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-white/25 bg-white/12 text-xs font-bold text-white press" aria-label={L.heroInfoAria}>
              !
            </button>
          </div>
      ...
      <section className="shrink-0 px-3 pb-2">
        <div className="relative overflow-hidden rounded-xl border-2 border-bank-gold/85 p-3 shadow-bankCard" style={{ background: INDIAN_BANK.summaryBg }}>
          ...
          <div className="grid grid-cols-2 gap-2">
            <SummaryCell icon={piggy} title={L.savingsShort} count={summary.savingsCount} amount={summary.savingsBal} visible={balancesVisible} maskText={maskText} />
            <SummaryCell
              icon={
                <svg viewBox="0 0 24 24" fill="none" aria-hidden>
      ...
      <section className="shrink-0 px-3 pb-2">
        <div className="grid auto-rows-fr grid-cols-4 gap-2">
          <GoldQuickTile icon={<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><rect x="5" y="4" width="14" height="16" rx="2" /><path d="M9 8h6M9 12h6" strokeLinecap="round" /></svg>} label={L.tileAccountDetails} onClick={() => onQuickAction('check_balance')} />
          <GoldQuickTile icon={<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M7 4h10v16H7z" /><path d="M9 8h6M9 11h4" strokeLinecap="round" /></svg>} label={L.tileAccountStatement} onClick={() => (onOpenTxnHistory ? onOpenTxnHistory() : onQuickAction('check_balance'))} />
          <GoldQuickTile icon={<span className="text-sm font-bold">m</span>} label={L.tileMPassbookQuick} onClick={() => onQuickAction('check_balance')} />
          <GoldQuickTile icon={<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M8 12h8M12 8v8" strokeLinecap="round" /><circle cx="12" cy="12" r="9" /></svg>} label={L.tileFundTransfers} onClick={() => (onFundTransferImps ? onFundTransferImps() : onQuickAction('internal_transfer'))} />
          <GoldQuickTile icon={<span className="text-[10px] font-black tracking-tighter">VBX</span>} label={L.tileVbxBenefits} onClick={() => onQuickAction('check_balance')} />
          <GoldQuickTile icon={<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M5 4h14v12H5z" /><path d="M9 20h6" strokeLinecap="round" /></svg>} label={L.tileCallback} onClick={() => onQuickAction('check_balance')} />
          <GoldQuickTile icon={<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><circle cx="12" cy="8" r="3" /><path d="M6 20v-2a4 4 0 014-4h4a4 4 0 014 4v2" /></svg>} label={L.tileVirtualRM} onClick={() => onQuickAction('check_balance')} />
          <GoldQuickTile icon={<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><rect x="4" y="6" width="16" height="12" rx="2" /><path d="M8 10h8" strokeLinecap="round" /></svg>} label={L.tilePriorityServiceCard} onClick={() => onQuickAction('check_balance')} />
        </div>
      </section>
      ...
      <section className="mt-auto flex min-h-0 flex-1 flex-col rounded-t-[22px] border-t border-white/20 bg-white px-3 pb-3 pt-3 text-ink shadow-[0_-10px_36px_rgba(0,0,0,0.38)]">
        ...
        <div className="grid auto-rows-fr grid-cols-4 gap-2">
          <ServiceTile icon={<TriColorUpiMark />} label={L.svcUPI} onClick={() => onQuickAction('send_money')} />
          <ServiceTile icon={<div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-blue-500 to-indigo-600 text-lg">{'\uD83C\uDFC5'}</div>} label={L.svcRewards} onClick={() => onQuickAction('check_balance')} />
          <ServiceTile icon={<div className="flex h-full w-full items-center justify-center bg-blue-700 text-white"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6"><path d="M8 4h12v16H8z" /><path d="M6 8H4v12h12v-2" /></svg></div>} label={L.svcOpenOd} onClick={() => onQuickAction('internal_transfer')} />
          <ServiceTile icon={<div className="flex h-full w-full items-center justify-center bg-sky-700 text-lg text-white">{'\uD83D\uDCB3'}</div>} label={L.svcDebitApply} onClick={() => onQuickAction('check_balance')} />
          <ServiceTile icon={<div className="flex h-full w-full items-center justify-center rounded-xl bg-blue-900 text-sm font-black text-white">B</div>} label={L.svcBillPay} onClick={() => onQuickAction('pay_bill')} />
          <ServiceTile icon={<div className="flex h-full w-full items-center justify-center bg-orange-500 text-xl">{'\uD83D\uDE97'}</div>} label={L.svcFastag} onClick={() => onQuickAction('pay_bill')} />
          <ServiceTile icon={<div className="flex h-full w-full items-center justify-center bg-orange-600 text-sm font-bold text-white">%</div>} label={L.svcNewLoan} onClick={() => (onApplyNewLoan ? onApplyNewLoan() : onQuickAction('check_balance'))} />
          <ServiceTile icon={<div className="flex h-full w-full items-center justify-center bg-emerald-600 text-xl">{'\uD83C\uDFE6'}</div>} label="Create Deposit" onClick={() => (onOpenDeposit ? onOpenDeposit() : onQuickAction('check_balance'))} />
          <ServiceTile icon={<div className="flex h-full w-full items-center justify-center bg-indigo-600 px-1 text-center text-[10px] font-bold leading-tight text-white">NCMC</div>} label={L.svcRupayWallet} onClick={() => onQuickAction('check_balance')} />
          <ServiceTile icon={<div className="flex h-full w-full items-center justify-center bg-sky-800 text-lg">{'\uD83D\uDCB3'}</div>} label={L.svcDebitCard} onClick={() => onNavigate?.('debit_card')} />
          <ServiceTile icon={<div className="flex h-full w-full items-center justify-center bg-blue-950 text-lg">{'\uD83D\uDCB3'}</div>} label={L.svcCreditCard} onClick={() => onNavigate?.('credit_card')} />
          <ServiceTile icon={<div className="flex h-full w-full items-center justify-center bg-violet-800 text-white"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6"><path d="M8 4h12v16H8z" /><path d="M6 8H4v12h12v-2" /></svg></div>} label={L.svcLoanAccounts} onClick={() => onQuickAction('check_balance')} />
          <ServiceTile icon={<div className="flex h-full w-full items-center justify-center bg-cyan-700 text-xl">{'\u2708'}</div>} label={L.svcTravelLeisure} onClick={() => onQuickAction('book_flight')} />
        </div>
      </section>
