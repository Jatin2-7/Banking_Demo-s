...
export function TextInput({
  ...
  return (
    <div
      className={`flex items-center gap-2 rounded-lg border border-incred-border bg-white px-3 py-2.5 ${className}`}
    >
...
export function DobInputs({ day, month, year, onDay, onMonth, onYear, hint }) {
  const box =
    'w-[4.5rem] rounded-lg border border-incred-border bg-white px-2 py-2.5 text-center text-[14px] outline-none placeholder:text-gray-400';
  return (
    <div>
      ...
      {hint && (
        <p className="mt-1 text-[12px] font-medium" style={{ color: INCRED.green }}>
          {hint}
        </p>
      )}
    </div>
  ...
  return (
    <div className="relative flex items-center gap-2 rounded-lg border border-incred-border bg-white px-3 py-2.5">
      ...
      >
        ...
        {options.map((o) => (
          <option
            key={typeof o === 'string' ? o : o[valueKey]}
  ...
  return (
    <label className="flex cursor-pointer items-start gap-2.5 press">
      <span
        ...
        style={{
          backgroundColor: checked ? INCRED.orange : 'white',
          border: checked ? 'none' : `2px solid ${INCRED.border}`,
        }}
      >
        {checked && (
          <svg
...
export function OrangeButton({ children, onClick, disabled, className = '', outline }) {
  const base =
    'w-full rounded-lg py-3.5 text-[15px] font-bold press transition-opacity disabled:opacity-50';
  if (outline) {
    return (
      <button
        type="button"
