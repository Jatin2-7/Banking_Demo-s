...
import { INCRED_HOME_AGENT_ID, INCRED_HOME_AGENT_SYSTEM } from './incredHomeAguiConfig.js';
import { module_ } from '../lib/log.js';
import { getOpenAIClient, getChatModel, hasLlmConfigured } from '../lib/openaiClient.js';

const log = module_('agui:incred_home');
...
const HOME_TOOLS = [
  {
    ...
    function: {
      name: 'navigate_to',
      description: 'Navigate the customer to an InCred screen or journey.',
      parameters: {
        type: 'object',
        properties: {
          destination: {
            type: 'string',
            enum: ['personal_loan', 'my_loans', 'profile', 'home'],
          },
          context: { type: 'string' },
        },
...
export async function streamIncredHomeAguiRun(res, agentId, inputData, { signal } = {}) {
  if (agentId !== INCRED_HOME_AGENT_ID) {
  ...
  try {
    ...
    log.error({ err: err?.message || String(err) }, 'incred home agui stream error');
