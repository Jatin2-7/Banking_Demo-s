...
import {
  DCB_GOLD_LOAN_AGENT_ID,
  DCB_GOLD_LOAN_AGENT_SYSTEM,
  DCB_GOLD_LOAN_EMPLOYMENT_OPTIONS,
  DCB_GOLD_LOAN_PURPOSE_OPTIONS,
} from './dcbGoldLoanConfig.js';
import { module_ } from '../lib/log.js';
import { getOpenAIClient, getChatModel, hasLlmConfigured } from '../lib/openaiClient.js';

const log = module_('agui:dcb-gold-loan');
...
const GOLD_LOAN_TOOLS = [
  {
    type: 'function',
    function: {
      name: 'set_field',
      description: 'Set a single gold loan form field.',
      parameters: {
        type: 'object',
        properties: {
          field: {
            type: 'string',
            enum: ['loanAmount', 'tenureMonths', 'goldWeightGrams', 'purpose', 'employment'],
          },
  ...
  },
  {
    ...
    function: {
      name: 'submit_loan',
      description: 'Open MPIN step after all fields are filled and customer confirmed.',
      parameters: { type: 'object', properties: {} },
    },
  },
];

function executeGoldLoanTool(name, args, state) {
  if (name === 'set_field') {
    const { field, value } = args;
    const validFields = ['loanAmount', 'tenureMonths', 'goldWeightGrams', 'purpose', 'employment'];
...
function buildSystemPrompt(state) {
  return `${DCB_GOLD_LOAN_AGENT_SYSTEM}\n\n## Current form state\n${JSON.stringify(state, null, 2)}`;
}

export async function streamDcbGoldLoanRun(res, agentId, inputData, { signal } = {}) {
  if (agentId !== DCB_GOLD_LOAN_AGENT_ID) {
  ...
  try {
    ...
    for (let step = 0; step < 14; step++) {
      ...
      for (const slot of toolCallBuf.values()) {
        ...
        const exec = executeGoldLoanTool(slot.name, args, state);
    ...
    log.error({ err: err?.message || String(err) }, 'dcb gold loan agui stream error');
