...
export async function streamImpsAguiRun(res, agentId, inputData, { signal } = {}) {
  if (agentId !== IMPS_AGENT_ID) {
  ...
  try {
    ...
    log.error({ err: err?.message || String(err) }, 'imps agui stream error');
    ...
    if (status === 401 || code === 'invalid_api_key') {
      ...
      hint = 'OpenAI rate limit exceeded. Wait a moment and try again.';
    }
