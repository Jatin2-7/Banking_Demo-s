...
const log = module_('agui:ss_quickloan');
...
function buildSystemPrompt(state) {
  ...
  return SS_QUICKLOAN_AGENT_SYSTEM + tail;
}

/**
 * Stream AG-UI-compatible SSE to Express response for the SilverSuits QuickLoan agent.
 */
export async function streamSSQuickLoanRun(res, agentId, inputData, { signal } = {}) {
  if (agentId !== SS_QUICKLOAN_AGENT_ID) {
  ...
  if (!hasLlmConfigured()) {
    ...
    res.write(
      sseEncode({
        type: 'RUN_ERROR',
        message:
          'LLM not configured. Add Azure OpenAI or OPENAI_API_KEY to the server .env file and restart.',
      }),
    );
    res.end();
    return;
  ...
  try {
    ...
    const tools = ssQuickLoanOpenAiTools();
    ...
    for (let step = 0; step < 14; step++) {
      ...
      // Refresh system prompt with updated state after each tool round
      messages[0] = { role: 'system', content: buildSystemPrompt(state) + systemTail };
    }

    write({ type: 'RUN_FINISHED', thread_id: threadId, run_id: runId });
  } catch (err) {
    log.error({ err: err?.message || String(err) }, 'ss_quickloan agui stream error');
