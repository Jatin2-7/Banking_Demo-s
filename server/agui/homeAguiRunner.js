    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          destination: {
            type: 'string',
            enum: ['upi_payment', 'fund_transfer', 'loan_application'],
    ...
    for (let step = 0; step < 8; step++) {
      ...
      if (toolCallBuf.size === 0) {
        ...
        if (/navigate_to/i.test(textLower)) {
          const destMatch = textLower.match(/destination\s*[=:]\s*["']?(upi_payment|fund_transfer|loan_application)["']?/i);
          const ctxMatch = textLower.match(/context\s*[=:]\s*["']([^"'\n]+)["']/i);
          if (destMatch) {
            const args = { destination: destMatch[1], context: ctxMatch?.[1] || '' };
            const destLabel = args.destination === 'upi_payment' ? 'UPI Payment'
              : args.destination === 'fund_transfer' ? 'IMPS Fund Transfer'
              : 'Loan Application';
            write({ type: 'STATUS_UPDATE', status: `Redirecting you to ${destLabel}…` });
      ...
      for (const slot of toolCallBuf.values()) {
        ...
        // Emit a routing status so the UI can show "Redirecting…"
        const destLabel = args.destination === 'upi_payment' ? 'UPI Payment'
          : args.destination === 'fund_transfer' ? 'IMPS Fund Transfer'
          : args.destination === 'loan_application' ? 'Loan Application'
          : args.destination;
        write({ type: 'STATUS_UPDATE', status: `Redirecting you to ${destLabel}…` });
